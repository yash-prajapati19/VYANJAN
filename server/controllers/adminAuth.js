...
const otpVerification = async (req, res) => {
  try {
    ...
    const foundAdmin = await AdminData.findOne({ email });
    const foundResturant = await resturantData.findOne({
      admin_id: foundAdmin._id,
    });
    const resturantItems = await itemsData.find({
      creator: foundResturant._id,
    });
    ...
    return res
      .status(200)
      .send({ auth: true, resturantItems, foundAdmin, foundResturant });
  } catch (error) {
    return res.status(404).json({ error: error });
  }
};
...
const registerAdmin_Resturant = async (req, res) => {
  try {
    const {
      ...
      resturantTags,
      imageUrl,
      resturantAddress: { street, aptName, locality, zip, lat, lng, phoneNo },
      // resturantItems,
      email,
      password,
    } = req.body;
